...
<head>
    ...
    <title><?php echo $row->page_name; ?> | Rooms</title>
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
			...
			<div class="row">
                <!-- Page Header -->
				<a href="javascript:add_this()"><i class="fa fa-plus fa-fw"></i> Add Room</a>
	...
	function add_this(){
		...
		var users_form = '\
		<form id="update_user_form" method="post" action="<?php echo $this->config->base_url().'rooms/add_room';?>">\
			<div class="row">\
				<div class="col-lg-6">\
			...
			<div class="row">\
				<div class="col-lg-6">\
					<div class="form-group">\
						<label>Color :</label>\
						<select class="form-control" id="color" name="color"><option value="green">Green</option><option value="blue">Blue</option><option value="red">Red</option><option value="white">White</option></select>\
					</div>\
				</div>\
				<div class="col-lg-6">\
					<div class="form-group">\
			...
			<div class="row">\
				<div class="col-lg-6">\
					<button type="button" class="ar-button btn btn-success">Save</button>\
		...
		setTimeout(function(){
			...
			.bootstrapValidator({
				...
				fields: {
					room_name: {
						validators: {
							notEmpty: {
								message: 'Field room name is required'
							}
						}
					}
				}
			...
			.on('success.form.bv', function(e,data) {
				...
				$.post($form.attr('action'), $form.serialize(), function(result) {
					alert(result.msg);
					window.location.href = "<?php echo $this->config->base_url().'rooms';?>";
				}, 'json');
			});
		}, 1000);
	}
	...
	function edit_this(theid){
		...
		$.get("<?php echo $this->config->base_url().'rooms/get_room_detail';?>/"+theid,function(result){
		
		
		var users_form = '\
		<form id="update_user_form" method="post" action="<?php echo $this->config->base_url().'rooms/update_room';?>/'+theid+'">\
			<div class="row">\
				<div class="col-lg-6">\
					<div class="form-group">\
						<label>Room Name :</label>\
						<input class="form-control" id="room_name" name="room_name" value="'+result.room_name+'"/>\
					</div>\
				</div>\
			</div>\
			<div class="row">\
				<div class="col-lg-6">\
					<div class="form-group">\
						<label>Color :</label>\
						<select class="form-control" id="color" name="color"><option '+(result.color=='green'?'selected':'')+' value="green">Green</option><option '+(result.color=='blue'?'selected':'')+' value="blue">Blue</option><option '+(result.color=='red'?'selected':'')+' value="red">Red</option><option '+(result.color=='white'?'selected':'')+' value="white">White</option></select>\
					</div>\
				</div>\
				<div class="col-lg-6">\
					<div class="form-group">\
			...
			<div class="row">\
				<div class="col-lg-6">\
					<button type="button" class="ur-button btn btn-success">Update</button>\
					<button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button>\
				</div>\
			</div>\
			</form>\
		...
		setTimeout(function(){
			$("body").mLoading("hide");
			$("#dynamic-modal-title").html('Update Room');
			$("#dynamic-modal-body").html(users_form);
			$("#dynamic-modal-footer").html('');
			$("#dynamic-modal").modal({show:true});
			
			$(document).on("click",".ur-button",function(){
	...
	function delete_this(theid){
		if(confirm("Are you sure you want to delete this room?")){
			$.get("<?php echo $this->config->base_url().'rooms/delete_room';?>/"+theid, function(result){
				alert(result.msg);
				window.location.href = "<?php echo $this->config->base_url().'rooms';?>";
			}, 'json');
		}else
	...
	$(document).ready(function(){
		...
		{   "bJQueryUI": true,
				...
				"bProcessing": true,
				"bServerSide": true,
			"sAjaxSource": "<?php echo $this->config->base_url().'rooms/room_listing';?>",
			"bFilter": true,
			 "sDom": '<"H"lfr>t<"F"ip>',
